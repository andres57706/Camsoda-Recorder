...
class Settings():
    def __init__(self, parser, make_absolute):
        ...
        self.recording_utility = parser.get(
            'settings', 'recording_utility').lower()
...
class RecordingThread(threading.Thread):
    ...
    def run(self):
        try:
            ...
            self.model_data['dl_path'] = self.create_path(
                self.config.settings.path_structure, start_time)
            self.model_data['stream'] = get().stream(
                model_data=self.model_data, r=self.r)
            os.makedirs(os.path.dirname(
                self.model_data['dl_path']), exist_ok=True)
